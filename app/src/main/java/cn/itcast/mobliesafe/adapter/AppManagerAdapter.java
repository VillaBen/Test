...
import cn.itcast.mobliesafe.utils.DensityUtil;
import cn.itcast.mobliesafe.utils.EngineUtils;
...
public class AppManagerAdapter extends BaseAdapter {
    ...
    public int getCount() {
        ...
        return UserAppInfos.size()+SystemAppInfos.size()+3;
    }

    @Override
    public Object getItem(int position) {
        if (position==0){
            // 第0个位置显示的应该是 用户程序的个数的标签行。
            return null;
        ...
        if (position<=(UserAppInfos.size())){
            // 用户程序
            appInfo=UserAppInfos.get(position-1);// 多了一个textview的标签,位置需要-1
        }else{
            // 系统程序
            int location=position-UserAppInfos.size()-2;// 多了两个textview的标签,位置需要-2
